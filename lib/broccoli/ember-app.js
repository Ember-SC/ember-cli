...
var broccoliPetal = require('broccoli-petal');
...
EmberApp.prototype.import = function(asset, options) {
  ...
  console.log("Asset: " + JSON.stringify(asset, null, '\t') + " Options: " + JSON.stringify(options, null, '\t') + 'Extension: ' + extension);
  ...
  if (options.exports) {
    if (!extension) {
      ...
      // deanonimize all files in assetPath dir see https://github.com/abuiles/broccoli-petal/blob/master/Brocfile.js#L3
      var petalAssetTree = broccoliPetal(assetPath, {
        name: options.remap
      });

      this._importTrees.push(petalAssetTree);
      ...
      console.log(legacyFilesToAppend);

      // detect exports with petal and add them to importWhitelist
    } else {
